...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        ...
        if (self.scope == FSCalendarScopeMaxHeight && self.transitionCoordinator.state == FSCalendarTransitionStateIdle) {
            rowHeight = [self effectiveMaxRowHeight];
        }
        CGFloat padding = 0;
        ...
        if (!self.floatingMode) {
            switch (self.transitionCoordinator.representingScope) {
                ...
                case FSCalendarScopeMaxHeight: {
                    CGFloat effectiveRowHeight = rowHeight;
                    if (self.scope == FSCalendarScopeMaxHeight && self.transitionCoordinator.state == FSCalendarTransitionStateIdle) {
...
- (CGSize)sizeThatFits:(CGSize)size scope:(FSCalendarScope)scope
{
    ...
    CGFloat monthRowHeight = [self baselineRowHeight];
    CGFloat maxRowHeight = [self effectiveMaxRowHeight];
    ...
    if (!self.floatingMode) {
        switch (scope) {
            ...
            case FSCalendarScopeWeek: {
                CGFloat height = weekdayHeight + headerHeight + monthRowHeight + paddings;
                return CGSizeMake(size.width, height);
            }
            case FSCalendarScopeMaxHeight: {
...
- (BOOL)floatingMode
{
    return (_scope == FSCalendarScopeMonth || _scope == FSCalendarScopeMaxHeight) && _scrollEnabled && !_pagingEnabled;
}
...
- (CGFloat)baselineRowHeight
{
    CGSize cachedSize = self.transitionCoordinator.cachedMonthSize;
    if (CGSizeEqualToSize(cachedSize, CGSizeZero)) {
        cachedSize = self.bounds.size;
    }
    CGFloat headerHeight = self.preferredHeaderHeight;
    CGFloat weekdayHeight = self.preferredWeekdayHeight;
...
- (BOOL)canTransitionToMaxHeight
{
    return (self.effectiveMaxRowHeight - [self baselineRowHeight]) > FLT_EPSILON;
}
